...
class FlxSprite extends FlxObject
{
	...
	{
        AssetTracking.release(cacheKey);
		super.destroy();
	...
	public function loadGraphic(graphic:FlxGraphicAsset, animated = false, frameWidth = 0, frameHeight = 0, unique = false, ?key:String):FlxSprite
	{
        ...
        //returns the graphic asset as the newly tracked graphic so it can still be set and used in a normal way - BeastTornado
        cacheKey = graphic;
		var graph:FlxGraphic = AssetTracking.get(graphic);
		if (graph == null)
			return this;
